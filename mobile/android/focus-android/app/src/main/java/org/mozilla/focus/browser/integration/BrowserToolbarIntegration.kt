...
class BrowserToolbarIntegration(
    ...
    private fun setBrowserActionButtons() {
        tabsCounterScope = coroutineScope.also {
            it.launch {
                store.stateFlow.distinctUntilChangedBy { state -> state.tabs.size > 1 }
                    .collect { state ->
    ...
    internal fun observeEraseCfr() {
        eraseTabsCfrScope = coroutineScope.also {
            it.launch {
                fragment.components?.appStore?.stateFlow?.mapNotNull { state -> state.showEraseTabsCfr }
    ...
    internal fun observeCookieBannerCfr() {
        cookieBannerCfrScope = coroutineScope.also {
            it.launch {
                fragment.components?.appStore?.stateFlow.mapNotNull { state -> state.showCookieBannerCfr }
                    .distinctUntilChanged()
    ...
    internal fun observeTrackingProtectionCfr() {
        trackingProtectionCfrScope =coroutineScope.also {
            it.launch {
                fragment.components?.appStore?.stateFlow?.mapNotNull { state -> state.showTrackingProtectionCfrForTab }
                    ?.distinctUntilChanged()
    ...
    internal fun observerSecurityIndicatorChanges() {
        securityIndicatorScope = coroutineScope.also {
            it.launch {
                store.stateFlow.mapNotNull { state -> state.findCustomTabOrSelectedTab(customTabId) }
